...
namespace Assets.Scripts.OpenStreetMap
{
    ...
    class Highway
    {
        ...
        private void generateInitial3Dway(Way way)
        {
	        ...
	        if (way.nodes.Count == 2)
	        {
                ...
                if (isTrafficLight(way.nodes[0]))
                    drawTrafficSign(leftSideVertexes[0]);
                if (isTrafficLight(way.nodes[0]))
                    drawTrafficSign(leftSideVertexes[1]);
	        ...
	        else
	        {
		        for (int i = 0; i < way.nodes.Count - 2; i++)
		        {
                    ...
                    if (isTrafficLight(way.nodes[i]))
                        drawTrafficSign(new Vector3(p0.x, terrain.getTerrainHeight2(p0.y + bbox.meterBottom, p0.x + bbox.meterLeft), p0.y));
        ...
        private void createHighway()
        {
            ...
            {
                vertices[k] = leftSideVertexes[m] + new Vector3(0,0.25f,0);
                vertices[k + 1] = rightSideVertexes[m] + new Vector3(0, 0.25f, 0);
            }
        ...
        {
                GameObject trafficSign = (GameObject)MonoBehaviour.Instantiate(Resources.Load("Prefabs/TrafficSign/TrafficSign"));
                trafficSign.transform.position = pos;              
        }
